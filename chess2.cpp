...
void printBoard()
{
    ...
    for (int i = 0; i < whiteCapturedCount; i++) {
        cout << capturedByWhite[i] << ' ';
    }

    cout << "\nBlack Captures: ";
    for (int i = 0; i < blackCapturedCount; i++) {
        cout << capturedByBlack[i] << ' ';
    }
...
int main()
{
    ...
    {
        if (count % 2 == 0){
            cout << "Enter White's Turn (e.g. 2a3a or exit): ";
            jump:
            cin >> play;
            if (strcmp(play, "exit") == 0)
                break;
            convertPlay();
            if (islower(board[rowFrom][colFrom]) || board[rowFrom][colFrom] == '.') {
                cout << "Invalid Move! It's White's turn.\nEnter again: ";
                goto jump;
            }
            if(checkTurn()==1){
                updateBoard();
                ...
                cout<<"Enter Valid Turn: ";
                goto jump;
            }
        }
        else{
            cout << "Enter Black's Turn (e.g. 7a6a or exit): ";
            jump1:
            cin >> play;
            if (strcmp(play, "exit") == 0)
                break;
            ...
            if (isupper(board[rowFrom][colFrom]) || board[rowFrom][colFrom] == '.') {
                cout << "Invalid Move! It's Black's turn.\nEnter again: ";
                goto jump1;
            }
            if(checkTurn()==1){
                updateBoard();
